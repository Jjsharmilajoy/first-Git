...
export default class Cancelled extends Component {
  static navigationOptions = props => ({
    title: props.navigation.getParam(
      'Cancelled ',
      `Cancelled (${props.screenProps.count ?
        props.screenProps.count.cancelled : 0})`
    ),
  ...
  componentDidMount() {
    this.props.navigation.setParams({
      onFocus: this.getCancelledLeads.bind(this)
    });
  }

  getCancelledLeads = () => {
    try {
      this.props.getTestRideBookedLeads('cancelled', this.state.dateRange).catch(() => { });
      this.props.screenProps.getCount('cancelled');
    } catch (error) {
